...
useEffect(() => {
  const loadAuthList = async () => {
    ...
    try {
      ...
      const response = await fetchData(api, `${common.getServerUrl("oper/usermngnew/list")}`, params);
      if (!response.success) {
  ...
  useEffect(() => {
    const initializeTable = async () => {
      ...
      try {
          ...
          [
            {
              ...
              formatter: (cell) => {
                ...
                checkbox.checked = rowData.isChanged === "Y";
                checkbox.onclick = () => {
                  setTimeout(() => {
                    setData((prevData) =>
                      prevData.map((row) => (row.EMPNO === rowData.EMPNO ? { ...row, isChanged: checkbox.checked ? "Y" : "N" } : row))
                    );
                  }, 0);
                };
            ...
            },
            {
              ...
              editable: false,
              cellClick: (e, cell) => {
                setSelectedEmpNo(cell.getRow().getData().EMPNO);
        ...
        setTableStatus("ready"); // 테이블 준비 완료로 상태 변경
        tableInstance.current.setData(data || []); // 초기 데이터 설정
      } catch (err) {
...
const handleSearch = async () => {
  ...
  try {
    ...
    const response = await fetchData(api, `${common.getServerUrl("oper/usermngnew/list")}`, params);
    console.log("Search Response:", response); // [추가: 디버깅 로그 추가]
    if (!response.success) {
  ...
  // [수정: API 경로를 oper/usermng/save에서 oper/usermng-new/save로 변경]
const handleSave = async () => {
  ...
  try {
    const promises = changedRows.map(async (row) => {
      ...
      try {
        const response = await fetchData(api, `${common.getServerUrl("oper/usermngnew/save")}`, params);
  ...
  return (
    <div className={styles.container}>
      ...
      <TableSearch
        ...
        rowCount={rowCount}
        onDownloadExcel={() => handleDownloadExcel(tableInstance.current, tableStatus, "사용자권한관리_NEW.xlsx")}
        buttonStyles={styles}
      >
      ...
      <CommonPopup show={showAuthPopup} onHide={handleAuthCancel} onConfirm={handleAuthConfirm} title="권한 선택">
        <div className="mb-3">
          <label htmlFor="authIdSelect" className="form-label">
            권한
          </label>
          <select
            className={`form-select ${styles.formSelect}`}
            id="authIdSelect"
            value={newAuth.AUTHID}
            onChange={(e) => {
          ...
          >
            ...
            {authList.map((auth) => (
              <option key={auth.AUTHID} value={auth.AUTHID}>
                {auth.AUTHNM}
              </option>
            ))}
          </select>
...
export default UserAuthManageNew;
